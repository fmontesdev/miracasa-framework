<?php
    class common {
        ...
        public static function load_model($model, $function = null, $args = null) {
            $dir = explode('_', $model);
            // constant() devuelve el valor de una constante previamente definida con define()
            // strtoupper() convierte un string a mayúsculas
            $path = constant('MODEL_' . strtoupper($dir[0])) .  $model . '.class.singleton.php';
            if (file_exists($path)) {
                require_once ($path);
                if (method_exists($model, $function)) { //comprueba si existe un método de una clase
        ...
        public static function generate_token_secure($longitud) {
            ...
            return bin2hex(openssl_random_pseudo_bytes(($longitud - ($longitud % 2)) / 2)); // genera token con gasto de recursos comedidos
        }

        function friendlyURL_php($url) {
            $link = "";
            if (URL_FRIENDLY) {
                // str_replace () reemplaza caràcteres del primer paràmetro por el segundo sobre la url
                $url = explode("&", str_replace("?", "", $url));
                foreach ($url as $key => $value) { // $key => $value indica aray asociativo
                    $aux = explode("=", $value);
                    $link .=  $aux[1]."/";
                }
            } else {
